...
metadata {
    ...
    // tile definitions
    tiles(scale: 2) {
        multiAttributeTile(name:"power", type: "generic", width: 6, height: 6)
        {
        ...
        standardTile("powerConsumption", "device.powerConsumption", inactiveLabel: false, decoration: "flat", width: 2, height: 2) 
        {
		...
		valueTile("Meter_reading_date", "device.Meter_reading_date", width: 2, height: 2) 
        {
            state "val", label: '검침일 : ${currentValue}일'
        }         
 		valueTile("Season", "device.Season", width: 2, height: 2) 
        {
            state "val", label: '시즌 : ${currentValue}'
        }      
        valueTile("Summer_season", "device.Summer_season", width: 2, height: 2) 
        {
            state "val", label: '섬머시즌일 : ${currentValue}일'
        }      
        valueTile("Etc_season", "device.Etc_season", width: 2, height: 2) 
        {
            state "val", label: '하계시즌일 : ${currentValue}일'
        }  
        valueTile("Electric_charges", "device.Electric_charges", width: 2, height: 2) 
        {
    ...
    if (event) 
    {
        ...
        if (event.name == "power") 
        {
 			...
 			else if (descMap.clusterInt == 0x0702 && descMap.attrInt == 0x0000) 
            {
                ...
                {
                   def value = event.value - device.currentState('resetTotal')?.doubleValue
                   sendEvent(name: "energy", value: value.round(3), unit: "kWh")
                   sendEvent(name: "kwhTotal", value:  event.value , unit: "kWh", displayed: false)               
                }         
            }            
        } 
        else if (event.name == "energy") 
        {
            ...
            else
            {
               def value = event.value - device.currentState('resetTotal')?.doubleValue
               sendEvent(name: "energy", value: value.round(3), unit: "kWh")
               sendEvent(name: "kwhTotal", value: value, unit: "kWh", displayed: false)               
            }
    ...
    else  
    {
        ...
        attrData.each 
        {
                ...
                if (it.clusterInt == 0x0702 && it.attrInt == 0x0000) 
                {
                        ...
                        {
                           def value = zigbee.convertHexToInt(it.value)/1000000 - device.currentState('resetTotal')?.doubleValue
                           sendEvent(name: "energy", value: value.round(6), unit: "kWh")
                           sendEvent(name: "kwhTotal", value:zigbee.convertHexToInt(it.value)/1000000, unit: "kWh", displayed: false)               
                        }                             
                }          
                if (map) 
                {
...
def this_month = (new Date().format("MM", location.timeZone))
    ...
    if( (this_month == '07') || (this_month == '09')) // 6~7 or 8~9
    {
       ...
       if(month_energy <= 200)
       {
          def temp_charge = Math.ceil(basic_fare+month_energy*93.3) 
          def temp_tax1 = temp_charge*0.1
          def temp_tax2 = temp_charge*0.037 
          
          sendEvent(name: 'Electric_charges', value: Math.ceil(temp_charge+temp_tax1+temp_tax2)  ) 
          sendEvent(name: 'powerConsumption_step', value: "계절/하계 누진1")          
       }
       else if(month_energy <=300)
       {
          def temp_charge = Math.ceil(               basic_fare + //기본 요금
    ...
    {     
       ...
       {
          def temp_charge = Math.ceil(basic_fare+(300*93.3)+((month_energy-300)*187.9))
          def temp_tax1 = temp_charge*0.1
          def temp_tax2 = temp_charge*0.037 
          sendEvent(name: 'Electric_charges', value: Math.ceil(temp_charge+temp_tax1+temp_tax2)  )      
       ...
       {
          def temp_charge = Math.ceil(basic_fare+(300*93.3)+(150*187.9)+((month_energy-450)*280.6))
          def temp_tax1 = temp_charge*0.1
          def temp_tax2 = temp_charge*0.037 
          sendEvent(name: 'Electric_charges', value: Math.ceil(temp_charge+temp_tax1+temp_tax2)  )      
       ...
       {
          def temp_charge = Math.ceil(basic_fare+(300*93.3)+(150*187.9)+(550*280.6)+((month_energy-1000)*709.5))
          def temp_tax1 = temp_charge*0.1
          def temp_tax2 = temp_charge*0.037 
          sendEvent(name: 'Electric_charges', value: Math.ceil(temp_charge+temp_tax1+temp_tax2)  )      
    ...
    else //1~6 , 9~12
    {
       ...
       {
          def temp_charge = Math.ceil(basic_fare+(200*93.3)+((month_energy-200)*187.9))
          def temp_tax1 = temp_charge*0.1
          def temp_tax2 = temp_charge*0.037 
          sendEvent(name: 'Electric_charges', value: Math.ceil(temp_charge+temp_tax1+temp_tax2)  )      
          sendEvent(name: 'powerConsumption_step', value: "현재 누진2")
       ...
       {
          def temp_charge = Math.ceil(basic_fare+(200*93.3)+(200*187.9)+((month_energy-400)*280.6))
          def temp_tax1 = temp_charge*0.1
          def temp_tax2 = temp_charge*0.037 
          sendEvent(name: 'Electric_charges', value: Math.ceil(temp_charge+temp_tax1+temp_tax2)  )      
          sendEvent(name: 'powerConsumption_step', value: "현재 누진3")        
       ...
       {
          def temp_charge = Math.ceil(basic_fare+(200*93.3)+(200*187.9)+(600*280.6)+((month_energy-1000)*709.5))
          def temp_tax1 = temp_charge*0.1
          def temp_tax2 = temp_charge*0.037 
          sendEvent(name: 'Electric_charges', value: Math.ceil(temp_charge+temp_tax1+temp_tax2)  )      
          sendEvent(name: 'powerConsumption_step', value: "현재 슈퍼누진4")          
