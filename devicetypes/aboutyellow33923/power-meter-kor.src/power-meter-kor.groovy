...
metadata {
    definition (name: "Power Meter KOR", namespace: "aboutyellow33923", author: "YooSangBeom", mnmn: "SmartThingsCommunity", vid:"9a5bd826-467c-3cd6-89f1-a2e29f29f902"){ 
    ...
    tiles(scale: 2) {
        multiAttributeTile(name:"power", type: "generic", width: 5, height: 5)
        {
            ...
            tileAttribute("device.powerConsumptionStep", key: "SECONDARY_CONTROL") 
            {
...
def this_month = (new Date().format("MM", location.timeZone))
    ...
    if( (this_month == '07') || (this_month == '09')) // 6~7 or 8~9
    {
       ...
       sendEvent(name: 'SummerSeason', value: Meter_reading_date.value)
       sendEvent(name: 'EtcSeason', value: (30 - ${Meter_reading_date.value}))    
       ...
       if(month_energy <= 200)
       {
          ...
          sendEvent(name: 'ElectricCharges', value: Math.round(temp_charge+temp_tax1+temp_tax2)  ) 
          sendEvent(name: 'powerConsumptionStep', value: "계절/하계 누진1")          
       }
       else if(month_energy <=300)
       {
          ...
          sendEvent(name: 'ElectricCharges', value: Math.round(temp_charge+temp_tax1+temp_tax2)  ) 
          sendEvent(name: 'powerConsumptionStep', value: "계절2 / 하계1")         
       }
       else if(month_energy <=400)
       {   
          def temp_charge = Math.round(               basic_fare + //기본 요금
       ...
       ((season_summer_energy - (200*season_summmer)) * 187.9))   //하계 누진2 요금
          ...
          sendEvent(name: 'ElectricCharges', value: Math.round(temp_charge+temp_tax1+temp_tax2)  ) 
          sendEvent(name: 'powerConsumptionStep', value: "계절2 / 하계2")          
       }
       ...
       ((season_summer_energy - (200*season_summmer)) * 187.9))   //하계 누진2 요금
          ...
          sendEvent(name: 'ElectricCharges', value: Math.round(temp_charge+temp_tax1+temp_tax2)  ) 
          sendEvent(name: 'powerConsumptionStep', value: "계절3 / 하계2")          
       }
       ...
       ((season_summer_energy - (200*season_summmer*2)) * 280.6)) //하계 누진3 요금
          ...
          sendEvent(name: 'ElectricCharges', value: Math.round(temp_charge+temp_tax1+temp_tax2)  ) 
          sendEvent(name: 'powerConsumptionStep', value: "계절3 / 하계3")        
       }
    }
    else if(this_month == '08') // 7~8
    {     
       sendEvent(name: 'Season', value: "하계시즌") 
       sendEvent(name: 'SummerSeason', value: 30) 
       sendEvent(name: 'EtcSeason', value: 0)        
    ...
    else //1~6 , 9~12
    {
       ...
       sendEvent(name: 'SummerSeason', value: 0) 
       sendEvent(name: 'EtcSeason', value: 30)
...
def updated() 
{
 	sendEvent(name: 'MeterReadingDate', value: MeterReadingDate) 
}
